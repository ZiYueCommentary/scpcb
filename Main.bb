...
Else
	For i% = 1 To TotalGFXModes
		...
		For  n% = 0 To TotalGFXModes - 1
			If GfxModeWidths(n) = GfxModeWidth(i) And GfxModeHeights(n) = GfxModeHeight(i) Then samefound = True 
			 Exit
		Next
...
Font2% = AALoadFont("GFX\font\cour\Courier New.ttf", Int(58 * (GraphicHeight / 1024.0)), 0,0,0)
...
ConsoleFont% = AALoadFont("GFX\font\cour\Courier New.ttf", Int(20 * (GraphicHeight / 1024.0)), 0,0,0,1)
			...
			Select Lower(StrTemp)
				...
				Case "playmusic"
					...
					If StrTemp$ <> ""
						...
						If CustomMusic <> 0 Then FreeSound_Strict CustomMusic 
						 CustomMusic = 0
								...
								If dist < 0.7 Then
									...
									If EntityPick(temp, 0.6) = d\buttons[i] Then
										If ClosestButton = 0 Then
											...
											ClosestDoor = d
										Else
											If dist < EntityDistance(Collider, ClosestButton) Then ClosestButton = d\buttons[i] 
											 ClosestDoor = d
										End If
									End If
			...
			If d\open Then
				...
				Else
					...
					If d\timerstate > 0 Then
						...
						If d\timerstate = 0 Then d\open = (Not d\open) 
...
Repeat
	...
	Else
			...
			If Rand(1500) = 1 Then
				For i = 0 To 5
					If AmbientSFX(i,CurrAmbientSFX)<>0 Then
						If ChannelPlaying(AmbientSFXCHN)=0 Then FreeSound_Strict AmbientSFX(i,CurrAmbientSFX) 
		...
		Color 255, 255, 255
		If ShowFPS Then AASetFont ConsoleFont 
			...
			If EndingTimer < -1000 And EndingTimer > -2000
				...
				If AchievementsMenu=0 Then
					...
					If DrawButton(x-145*MenuScale,y-100*MenuScale,390*MenuScale,60*MenuScale,"MAIN MENU", True)
						...
						NowPlaying = ShouldPlay
						For i=0 To 9
							If TempSounds[i]<>0 Then FreeSound_Strict TempSounds[i] 
							 TempSounds[i]=0
						Next
						StopStream_Strict(MusicCHN)
	...
	Else
		...
		If (Not IsZombie%)
			...
			Else If Playable Then
				If KeyDown(KEY_LEFT) Then angle = 90 
				 temp = True
				If KeyDown(KEY_RIGHT) Then angle = -90 
				 temp = True
			EndIf
		Else
			temp=True
				...
				Case "cup"
					...
					If CanUseItem(False,False,True)
						...
						Else
							...
							it\Picked = True
							For i = 0 To MaxItemAmount-1
								If Inventory(i)=SelectedItem Then Inventory(i) = it 
								 Exit
							Next
							EntityType (it\collider, HIT_ITEM)
									...
									Else
										If (Not ChannelPlaying(RadioCHN(0)))
											If (Not UserTrackFlag%)
												...
												EndIf
											EndIf
											If CurrUserTrack%<>0 Then FreeSound_Strict(CurrUserTrack%) 
											 CurrUserTrack% = 0
							...
							If SelectedItem\itemtemplate\tempname = "veryfineradio" Then ;"KOODIKANAVA"
								...
								If RadioState(6)-FPSfactor =< RadioState(7)*50 And RadioState(6)>RadioState(7)*50 Then
									...
									If RadioState(7)=>temp Then
										...
										If RadioState(8)=4 Then RadioState(8)=0 
										 RadioState(6)=-200
									EndIf
								EndIf
			...
			Select OptionsMenu
				...
				Case 4 ;Advanced
					...
					If AATextEnable_Prev% <> AATextEnable
						...
						EndIf
						InitAAFont()
						Font1% = AALoadFont("GFX\font\cour\Courier New.ttf", Int(16 * (GraphicHeight / 1024.0)), 0,0,0)
						Font2% = AALoadFont("GFX\font\courbd\Courier New.ttf", Int(56 * (GraphicHeight / 1024.0)), 0,0,0)
						Font3% = AALoadFont("GFX\font\DS-DIGI\DS-Digital.ttf", Int(20 * (GraphicHeight / 1024.0)), 0,0,0)
						Font4% = AALoadFont("GFX\font\DS-DIGI\DS-Digital.ttf", Int(58 * (GraphicHeight / 1024.0)), 0,0,0)
						Font5% = AALoadFont("GFX\font\Journal\Journal.ttf", Int(56 * (GraphicHeight / 1024.0)), 0,0,0)
						ConsoleFont% = AALoadFont("GFX\font\cour\Courier New.ttf", Int(20 * (GraphicHeight / 1024.0)), 0,0,0,1)
						;ReloadAAFont()
...
Function InitLoadGame()
	...
	For rt.RoomTemplates = Each RoomTemplates
		If rt\obj <> 0 Then FreeEntity(rt\obj) 
		 rt\obj = 0
	Next
...
Function UpdateMTF%()
	...
	If MTFtimer = 0 Then
		If Rand(30)=1 And PlayerRoom\RoomTemplate\Name$ <> "dimension1499" Then
			...
			For r.Rooms = Each Rooms
				If Lower(r\RoomTemplate\Name) = "gateaentrance" Then entrance = r 
				 Exit
			Next
...
Function UpdateDecals()
	...
	For d.Decals = Each Decals
		If d\SizeChange <> 0 Then
			...
			If d\Size >= d\MaxSize Then d\SizeChange = 0 
			 d\Size = d\MaxSize
		End If
...
Function CatchErrors(location$)
	...
	SetErrorMsg(0, "An error occured in SCP - Containment Breach!")
	SetErrorMsg(1, "Version: "+VersionNumber)
	SetErrorMsg(2, "Save compatible version: "+CompatibleNumber)
	SetErrorMsg(3, "Date and time: "+CurrentDate()+" at "+CurrentTime())
	SetErrorMsg(4, "Total video memory (MB): "+(TotalVidMem() / 1024))
	SetErrorMsg(5, "Available video memory (MB): "+(AvailVidMem() / 1024))
	SetErrorMsg(6, "Global memory status: " + ((TotalPhys() / 1024) - (AvailPhys() / 1024)) + " MB/" + (TotalPhys() / 1024) + " MB")
	SetErrorMsg(7, "Triangles rendered: "+CurrTrisAmount)
	SetErrorMsg(8, "Active textures: "+ActiveTextures()) ; _CaughtError_是一个宏，在显示时会被替换为引擎最后抛出的异常
	SetErrorMsg(9, location+": _CaughtError_")
End Function
